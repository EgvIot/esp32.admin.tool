...
// ------------------------------------------------------
//   Configuración de los parámetros WIFI
//-------------------------------------------------------

boolean settingsReadWifi(){
    StaticJsonDocument<500> jsonConfig;                             // se crea un JSON
    File file = SPIFFS.open("/SettingWifi.json", "r");
    if(deserializeJson(jsonConfig, file)){
        // Si falla la lectura, inicia valores por defecto
        settingResetWiFi();
        log("Error: Fallo la lectura de la configuración WiFi, tomado valores por defecto");
        return false;
    }else{
        /*---------------------General-------------------------------*/
