...
PubSubClient mqttclient(espClient);  //necesario para conectarse al broker
...
void callback(char *topic, byte *payload, unsigned int length){
  ...
  for(uint16_t i = 0; i < length; i++){
    ...
    mqttRX();                       //funcion de parpadeo en RX de datos
  }

  mensaje.trim();               //limpiar espacios que pueda contener el mensaje

  DynamicJsonDocument jsonDoc(300);


    /*La serialización es el proceso de convertir un objeto en una secuencia de bytes para 
    almacenarlo o transmitirlo a la memoria, a una base de datos o a un archivo. Su propósito 
    principal es guardar el estado de un objeto para poder volver a crearlo cuando sea necesario. 
